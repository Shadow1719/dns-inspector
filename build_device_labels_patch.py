...
if old not in text: raise SystemExit('device label patch failed: legacy device SELECT not found')
...
new = '''UPDATE devices SET name=COALESCE(NULLIF(name,''),?), hostname=COALESCE(NULLIF(hostname,''),?), mac=?,
                            ...
                            WHERE device_key=?'''
if old not in text: raise SystemExit('device label patch failed: legacy device UPDATE not found')
text = text.replace(old, new, 1)
...
if old not in text: raise SystemExit('device label patch failed: legacy device UPDATE args not found')
text = text.replace(old, new, 1)
...
new = '''INSERT INTO devices(device_key,name,hostname,mac,device_type,icon,confidence,source,first_seen,last_seen,request_count,info_json,label)
                             VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)'''
if old not in text: raise SystemExit('device label patch failed: legacy device INSERT not found')
text = text.replace(old, new, 1)
...
if old not in text: raise SystemExit('device label patch failed: legacy device INSERT args not found')
text = text.replace(old, new, 1)
...
if old not in text: raise SystemExit('device label patch failed: client_display SELECT not found')
text = text.replace(old, new, 1)
...
if old not in text: raise SystemExit('device label patch failed: client_display unpack not found')
text = text.replace(old, new, 1)
...
if old not in text: raise SystemExit('device label patch failed: client_display payload not found')
text = text.replace(old, new, 1)
...
if old not in text: raise SystemExit('device label patch failed: client_display fallback not found')
text = text.replace(old, new, 1)
...
if old not in text: raise SystemExit('device label patch failed: get_clients SELECT not found')
text = text.replace(old, new, 1)
...
if old not in text: raise SystemExit('device label patch failed: get_clients unpack not found')
text = text.replace(old, new, 1)
...
if old not in text: raise SystemExit('device label patch failed: get_clients payload not found')
text = text.replace(old, new, 1)
...
if old not in text: raise SystemExit('device label patch failed: device_detail SELECT not found')
text = text.replace(old, new, 1)
...
if old not in text: raise SystemExit('device label patch failed: device_detail payload not found')
text = text.replace(old, new, 1)
...
if old not in text: raise SystemExit('device label patch failed: detail primary not found')
text = text.replace(old, new, 1)
...
if old not in text: raise SystemExit('device label patch failed: detail HTML hook not found')
text = text.replace(old, new, 1)
...
if old not in text: raise SystemExit('device label patch failed: real_device_label loop not found')
text = text.replace(old, new, 1)

marker = '.clickable-label{cursor:pointer}.glance-meta{display:flex;flex-wrap:wrap;gap:6px;align-items:center;margin-top:4px;color:#8b949e;font-size:.78rem}.glance-devices{max-width:520px}'
...
if marker not in text: raise SystemExit('device label patch failed: CSS marker not found')
text = text.replace(marker, insert, 1)

# Browser MAC lookup uses the working public lookup route. The old macvendors.com/<mac> path is the API route and returns 404 in a browser.
text = text.replace('https://macvendors.com/${encodeURIComponent(c.mac)}', 'https://maclookup.app/search/result?mac=${encodeURIComponent(c.mac)}')
old = 'return f"https://macvendors.com/{quote(mac, safe=\':\') }" if mac else \'\''
new = 'return f"https://maclookup.app/search/result?mac={quote(mac, safe=\':\')}" if mac else \'\''
if old not in text: raise SystemExit('device label patch failed: server MAC lookup helper not found')
text = text.replace(old, new, 1)
...
if old not in text: raise SystemExit('device label patch failed: JS realDeviceLabel not found')
text = text.replace(old, new, 1)
...
if old not in text: raise SystemExit('device label patch failed: JS deviceRow return not found')
text = text.replace(old, new, 1)
text = text.replace('${esc(d.name)}', '${esc(d.label || d.name)}', 1)
...
if marker not in text: raise SystemExit('device label patch failed: renderClients marker not found')
text = text.replace(marker, insert, 1)
...
if old not in text: raise SystemExit('device label patch failed: renderClients binding hook not found')
text = text.replace(old, new, 1)
...
@app.route("/device")
def device_view():'''
if marker not in text: raise SystemExit('device label patch failed: device route marker not found')
text = text.replace(marker, insert, 1)
