...
| `GEOIP_DB_PATH` | `/data/geoip_country_ranges.csv` | Local/offline GeoIP CSV database for the DNS Destinations map. Same `/data` volume as `DB_PATH` above -- drop the converted CSV into your existing mounted data directory. No database ships by default -- see [`docs/GEOIP.md`](docs/GEOIP.md) |
...
| `GEOIP_CITY_DB_PATH` | `/data/geoip_city_ranges.csv` | Optional local/offline coordinate/city GeoIP CSV database for the map's Destinations mode. Same `/data` volume as `DB_PATH` above. No database ships by default -- see [`docs/GEOIP.md`](docs/GEOIP.md) |
