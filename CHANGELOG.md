...
## [Unreleased] - 0.8.6-dev.8 report period selector + PDF layout polish (Issue #88 follow-up)

- added an explicit **Report period** selector next to Export PDF on the Analytics tab (1h/6h/24h/7d/30d/90d presets plus a Custom From/To picker), fully independent of the dashboard's own live chart range -- exporting a 7d or custom report no longer changes, and is never silently overridden by, whatever range the Analytics chart happens to be showing
- the selected report period now drives the PDF's title, cover metadata, every KPI/chart, and the generated filename; a `ReportRangeError` (missing/backwards/too-short custom range) returns a clean `400` instead of a `500`
- when the requested report period exceeds retained query history, the PDF now shows an explicit "coverage note" banner instead of silently presenting a shorter window as complete
- fixed the "Most queried domains" / "Most active devices" PDF panels drawing labels underneath/behind their bars: labels now always sit on their own line above a bar, long domain/device names are ellipsized to a real pixel budget instead of a naive character slice, and a panel shows fewer rows rather than overlapping one when its height can't fit all of them
- redesigned the page-3 "Investigation prompt" callout: removed the "CLICKABLE TIMELINE" pseudo-button (a static PDF can't itself be clicked -- that behavior belongs in the UI), replaced with a plain "Investigate this interval in DNS Inspector" note, and laid the card out as four independently-spaced rows (heading / sentence / note / follow-up) that cannot overlap regardless of text length
- reworked the page-2 "What stands out" layout so the optional coverage banner grows/shrinks the space above the insight cards instead of shifting the cards down, which previously could push them into the donut/bar-list cluster below
