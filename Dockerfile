...
COPY build_perf_patch.py build_perf_hardcap.py build_memory_patch.py build_device_labels_patch_v2.py build_enrichment_retry_patch.py /app/
RUN python /app/build_perf_patch.py && python /app/build_perf_hardcap.py && python /app/build_memory_patch.py && python /app/build_device_labels_patch_v2.py && python /app/build_enrichment_retry_patch.py && rm /app/build_perf_patch.py /app/build_perf_hardcap.py /app/build_memory_patch.py /app/build_device_labels_patch_v2.py /app/build_enrichment_retry_patch.py
RUN mkdir -p /data
