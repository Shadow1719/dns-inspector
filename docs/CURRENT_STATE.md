...
0.8.5.2 is a front-end-only UI polish, layered on the same `HTML` template: the existing Analytics dashboard (Visual 2.0, Dashboard Builder, live activity, instrument gauges, the DNS Destinations map, history) is now the first tab in the nav bar and the app's default landing view, presented to users as **Home**. Only the visible label changed -- `data-tab="analytics"`, `id="tab-analytics"`, `/api/analytics*` routes and every other internal `analytics` identifier are untouched, so this is not the future BEMO-wide Home. Landing-tab selection (search/inspect -> Overview, an explicit "Default view" preference, otherwise the last-viewed tab) is unchanged in behaviour; only the tie-breaker for a first-ever visit with no saved tab and no explicit preference moved from Overview to Home. Domains/Devices remain unchanged as their own tabs for drill-down.
