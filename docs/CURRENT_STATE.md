...
Issue #31 (post-0.8.5.1 fix): `renderDestinationMap()` originally built the not-configured/zero-geolocated/zero-plotted states as bare `<div class="empty-state">` text with no `<svg>` at all, so an operator without a configured GeoIP database saw no map visualization whatsoever, not merely an unpopulated one. A small bundled/offline low-poly world landmass outline (`WORLD_LAND_MASSES`, `worldMapLandPaths()`, `app.py`) reusing the existing `mapProject()` equirectangular projection now renders as a shared `baseSvg()` base map in every branch of `renderDestinationMap()`; GeoIP configuration/data now only ever controls the destination-bubble overlay drawn on top of that base map, never whether the map itself is present. No network fetch, external tile, or CDN dependency was introduced, and the GeoIP data/API architecture above is otherwise unchanged.
