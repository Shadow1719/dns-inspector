...
/* Issue #63: a monotonic request token so an out-of-order/late network
   response (the 5-10s poll cycle can overlap a slow request with a faster
   later one) can never overwrite the map with stale data -- only the
   response to the most recently issued request is ever rendered. Click/
   keyboard selection never goes through this function at all, so it never
   waits on or is blocked by this polling cycle either. */
let mapFetchSeq = 0;
async function fetchDestinationMap(){
  ...
  try{
    ...
    if (seq !== mapFetchSeq) return; // a newer request already resolved
    renderDestinationMap(payload);
