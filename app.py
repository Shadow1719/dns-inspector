...
class ReportRangeError(ValueError):
    """Raised for an invalid/unsatisfiable custom report period request (never a 500)."""


REPORT_CUSTOM_MAX_SECONDS = _ANALYTICS_BUCKET_RANGES["90d"][0]
REPORT_CUSTOM_MIN_SECONDS = 300


def _custom_report_query_series(start_dt, end_dt):
    ...
    for point in points:
        bucket_end = _parse_iso(point["t"]) + timedelta(seconds=bucket_seconds)
        if min_dt is None or bucket_end <= min_dt: point["count"] = None
    return {"range": "custom", "label": label, "bucket_seconds": bucket_seconds, "points": points}, min_dt


def _report_window_coverage(start_dt, end_dt, retained_min_dt, range_key):
...
def report_payload(range_key="1h", from_param=None, to_param=None):
    """Build the analytics payload for a report/export period, decoupled from the
    dashboard's own `analyticsRange`. Presets (1h..90d) reuse `analytics_payload`
    (and therefore the same bounded analytics_buckets 30d/90d rollups the dashboard
    uses); "custom" resolves an explicit From/To window on the raw processed_queries
    timeline, independent of both the dashboard range and the preset bucket tables.
    """
    now_dt = datetime.now(timezone.utc)
    if range_key == "custom":
        ...
        queries_series, retained_min_dt = _custom_report_query_series(start_dt, end_dt)
        analytics = dict(analytics_payload("90d"))
        analytics["range"] = "custom"; analytics["range_options"] = REPORT_RANGE_OPTIONS
        analytics["series"] = dict(analytics.get("series") or {}); analytics["series"]["queries"] = queries_series
        window = {
            "range_key": "custom", "label": queries_series["label"],
            "start": start_dt.isoformat(), "end": end_dt.isoformat(),
            "filename_part": f"custom-{start_dt.strftime('%Y%m%d%H%M')}-{end_dt.strftime('%Y%m%d%H%M')}",
            "coverage": _report_window_coverage(start_dt, end_dt, retained_min_dt, "custom"),
        }
        return analytics, window
    if range_key not in ANALYTICS_RANGE_OPTIONS: range_key = "1h"
    analytics = analytics_payload(range_key)
    seconds = (_ANALYTICS_BUCKET_RANGES.get(range_key) or _ANALYTICS_RANGES.get(range_key))[0]
    start_dt, end_dt = now_dt - timedelta(seconds=seconds), now_dt
    label = ((analytics.get("series") or {}).get("queries") or {}).get("label", range_key)
    with closing(sqlite3.connect(DB_PATH)) as c:
        retained_min_dt = _parse_iso(c.execute("SELECT MIN(seen_at) FROM processed_queries").fetchone()[0]) if range_key not in _ANALYTICS_BUCKET_RANGES else \
            _parse_iso(c.execute("SELECT MIN(bucket_start) FROM analytics_buckets").fetchone()[0])
    window = {
        "range_key": range_key, "label": label,
        "start": start_dt.isoformat(), "end": end_dt.isoformat(),
        "filename_part": range_key,
        "coverage": _report_window_coverage(start_dt, end_dt, retained_min_dt, range_key),
    }
    return analytics, window


REPORT_RANGE_OPTIONS = ANALYTICS_RANGE_OPTIONS + ["custom"]
...
def generate_analytics_report_pdf(range_key, from_param=None, to_param=None):
    ...
    `range_key`/`from_param`/`to_param` describe the report's own export
    period, which is deliberately independent of whatever range the live
    Analytics dashboard happens to be showing (Issue #88 report-period follow-up)."""
    if range_key not in REPORT_RANGE_OPTIONS:
        range_key = "1h"
    analytics, window = report_payload(range_key, from_param, to_param)
    ...
    return report_buffer.getvalue(), {"range_key": range_key, "label": window["label"], "generated_at": utcnow(), "filename_part": window["filename_part"]}
...
def api_analytics_report_pdf():
    ...
    if range_key not in REPORT_RANGE_OPTIONS:
        range_key = "1h"
    try:
        pdf_bytes, meta = generate_analytics_report_pdf(range_key, request.args.get("from"), request.args.get("to"))
        stamp = datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S")
        return send_file(
            io.BytesIO(pdf_bytes),
            mimetype="application/pdf",
            as_attachment=True,
            download_name=f"dns-inspector-analytics-{meta['filename_part']}-{stamp}.pdf",
        )
